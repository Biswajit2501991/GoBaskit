...
export default function DiscountRibbon({ percent, className = '' }: DiscountRibbonProps) {
  ...
  return (
    <div className={`absolute top-0 left-2 z-10 select-none pointer-events-none ${className}`}>
      <div
        className="discount-ribbon bg-blinkit-green text-white text-center px-1.5 pt-1 pb-3 shadow-md max-sm:px-1 max-sm:pb-2"
        style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 50% 78%, 0 100%)' }}
      >
        <span className="block text-[11px] max-sm:text-[9px] font-extrabold leading-none">{percent}%</span>
        <span className="block text-[8px] max-sm:text-[7px] font-bold leading-none mt-0.5 tracking-wider">OFF</span>
      </div>
    </div>
  );
}
