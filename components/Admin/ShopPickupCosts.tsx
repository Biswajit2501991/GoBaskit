...
export default function ShopPickupCosts({
  ...
  async function markPaid(row: ShopPickupRow) {
    setSavingId(row.id);
    setError('');
    try {
      const res = await fetch(`/api/admin/shop-fulfillments/${row.id}/payment`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(typeof data.error === 'string' ? data.error : 'Could not mark paid');
        return;
      }
      if (data.shopSourcing) onSaved(data.shopSourcing);
    } finally {
  ...
  return (
    <div className={compact ? 'text-[11px] text-gray-700 space-y-1 pt-1 border-t border-gray-50' : 'space-y-3'}>
      ...
      {fulfillments.map((row) => {
        ...
        const payPending = row.paymentStatus === 'PENDING';
        const values = valuesFor(row);
        ...
        return (
          <div
            key={row.id}
            className={`${compact ? 'space-y-1' : 'bg-gray-50 rounded-lg p-3 space-y-2'} ${payPending ? 'ring-2 ring-amber-400 bg-amber-50' : ''}`}
          >
            <p className="font-medium">
              ...
              {payPending && (
                <span className="ml-1 text-red-600 font-semibold">Pending payment to shop</span>
              )}
              {row.paymentStatus === 'PAID' && (
                <span className="ml-1 text-emerald-700 font-semibold">Paid</span>
              )}
            </p>
