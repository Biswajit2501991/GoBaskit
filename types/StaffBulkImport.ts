...
/** Roles allowed in bulk import (All Super Admin must be created manually). */
export const BULK_ASSIGNABLE_STAFF_ROLES: StaffRole[] = STAFF_ROLES.filter(
  (r) => r !== 'ALL_SUPER_ADMIN',
);
...
export const STAFF_TEMPLATE_COLUMNS: StaffTemplateColumn[] = [
  ...
  { key: 'role', header: 'Role', description: `One of: ${BULK_ASSIGNABLE_STAFF_ROLES.join(', ')}`, sample: 'ORDER_MANAGER', required: true },
...
export function isValidStaffRole(role: string): role is StaffRole {
  return BULK_ASSIGNABLE_STAFF_ROLES.includes(role as StaffRole);
}
